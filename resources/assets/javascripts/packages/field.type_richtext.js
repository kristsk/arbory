...
jQuery(function () {
    ...
    var removeToolbarItem = function (toolbarConfig, removableItem) {
        for (var i = 0; i < toolbarConfig.length; i++) {
            removeArrayValue(toolbarConfig[i], removableItem);
        }
    };

    body.on('richtextinit', 'textarea.richtext', function (event, extraConfig) {
        ...
        if (textarea.data('attachment-upload-url')) {
            ...
            removeToolbarItem(config.toolbar, 'Image');
        }
        ...
        if (extraConfig) {
            jQuery.each(extraConfig, function (index, value) {
                config[index] = value;
            });
        }
    ...
    body.on('contentbeforeremove', function (e) {
        ...
        var removableItem = jQuery(e.target);
        var textareas = removableItem.is('textarea.richtext') ?
                        removableItem :
                        removableItem.find('textarea.richtext');
    ...
    // to avoid losing content ckeditor needs to be disabled and
    // re-enabled when used inside a sortable list
    ...
    body.on('beforeattributechange', 'textarea.richtext', function (event, eventParams) {
        if (eventParams.attribute !== 'id') {
            return;
        }
        jQuery(this).trigger('richtextsuspend');
    });

    body.on('attributechanged', 'textarea.richtext', function (event, eventParams) {
        if (eventParams.attribute !== 'id') {
            return;
        }
        jQuery(this).trigger('richtextresume');
